...
def writeToExcel(df, sheetNames, path, fileName, draw):
	...
	if draw:
		...
		print("Writing to: ", path+fileName+'_Match_Not_Generic.xlsx')
		writer = pd.ExcelWriter(path+fileName+'_Match_Not_Generic.xlsx', engine='xlsxwriter')
...
def grabList(date, monthArray):
	...
	if( uDay > 31 and (month == 1 or month == 3 or month == 5 or month == 7 or month == 8 or month == 10)):
			uDay = uDay - 31
			uMonth = 1 + month
	elif( uDay > 30 and (month == 4 or month == 6 or month == 9 or month == 11)):
			uDay = uDay - 30
			uMonth = 1 + month
	elif (uDay > 31 and month == 12):
		uDay = 31
	
	uDate = datetime(year, uMonth, uDay)
	lDate = datetime(year, lMonth, lDay)
	...
	for i in range(len(monthArray)):
		...
		if ('/' in  str(secondDate)):
			...
			secondYear = int(secondDate[len(secondDate) - 4:])
		elif ('-' in str(secondDate)):
			secondDay = int(secondDate.day)
			secondMonth = int(secondDate.month)
			secondYear = int(secondDate.year)

		qbDate = datetime(secondYear, secondMonth, secondDay)

		if ((qbDate <= uDate) and ( qbDate >= lDate) ):
